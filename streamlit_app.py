...
# Initialize with clear fields on first load
if 'app_initialized' not in st.session_state:
    st.session_state.app_initialized = True
    st.session_state.project_name = ''
    st.session_state.project_date = datetime.now().date()
    st.session_state.designer = ''
    st.session_state.project_description = ''
    st.session_state.contact_info = ''
    st.session_state.primary_weight = 0.0
...
def create_new_project():
    """Create a new project and reset form"""
    st.session_state.current_project_id = None
    st.session_state.project_counter += 1
    
    # Clear all project info fields
    st.session_state.project_name = ''
    st.session_state.project_date = datetime.now().date()
    st.session_state.designer = ''
    st.session_state.project_description = ''
...
# Create tabs
tab1, tab2, tab3, tab4 = st.tabs(["📊 Calculator", "📋 All Results", "📁 Project Results", "⚙️ Data Manager"])
...
with tab1:
    ...
    with col2:
        designer = st.text_input(
            "Designer",
            value=st.session_state.get('designer', ''),
        ...
        contact = st.text_input(
            "Contact Info",
            value=st.session_state.get('contact_info', ''),
...
with tab3:
    ...
    if st.session_state.projects:
        ...
        # Project selector
        project_options = {f"{p['project_number']} - {p['project_name']}": p['project_number'] 
        ...
        with col1:
            if st.button("📂 Load Selected Project", use_container_width=True):
                project_num = project_options[selected_project]
                load_project(project_num)
                st.success(f"✅ Loaded project {project_num}")
                time.sleep(1)
        
        with col2:
            if st.button("🗑️ Delete Selected Project", use_container_width=True):
                project_num = project_options[selected_project]
                st.session_state.projects = [p for p in st.session_state.projects 
                                            if p['project_number'] != project_num]
                save_projects()
                st.success(f"✅ Deleted project {project_num}")
                time.sleep(1)
                st.rerun()
        
        # Show detailed view of selected project
        if selected_project:
            st.markdown("---")
            st.markdown("### Project Details")
            
            project_num = project_options[selected_project]
            project = next((p for p in st.session_state.projects if p['project_number'] == project_num), None)
            ...
            if project:
                ...
                with col2:
                    ...
                    **Primary Product:**  
                    ...
                    Volume: {results['mm³']:,.2f} mm³ / {results['cm³']:,.2f} cm³ / {results['in³']:,.3f} in³
                    """)
                    
                    # Box volume if available
                    if project.get('box_volume_mm3', 0) > 0:
                        remaining = project['box_volume_mm3'] - project['primary_volume_mm3']
                        st.info(f"""
                        **Secondary Packaging:**  
                        Box: {project['box_length']} × {project['box_width']} × {project['box_height']} {project['dimension_unit']}  
                        Box Volume: {project['box_volume_mm3']:,.2f} mm³  
                        Remaining Space: {remaining:,.2f} mm³
                        """)
    else:
